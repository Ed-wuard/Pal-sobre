...
function writeMonth(month, listJson) {
	...
	for (var i = 0; i >= dateInput.length - 1; i++) {
		...
		console.log(i+'kkkk'+ dateInput.length);
	...
	for (var i = 1; i <= getTotalDays(month); i++) {
		if (month === fixedCurrentDate.getMonth()) {
			for (var a = 0; a <= dateInput.length - 1; a++) {
				var dateInputOne = new Date(dateInput[a].value);
				var dayInputOne= dateInputOne.getDate();

				if (i === Math.abs(dayInputOne)) {
					dateDays[f] = `<div class="calendar-date calendar-item-day yes days"><div class="c-v">${i}</div></div>`;
				}else {
